...
~/go/bin/seid init demo --chain-id $chainid
...
~/go/bin/seid add-genesis-account $(~/go/bin/seid keys show $keyname -a --keyring-backend test) 100000000000000000000usnp --keyring-backend test
# gentx for account
~/go/bin/seid gentx $keyname 7000000000000000usnp --chain-id $chainid --keyring-backend test
...
cat ~/.snp/config/genesis.json | jq '.app_state["gov"]["deposit_params"]["max_deposit_period"]="60s"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["gov"]["voting_params"]["voting_period"]="30s"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["gov"]["voting_params"]["expedited_voting_period"]="10s"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["oracle"]["params"]["vote_period"]="2"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["oracle"]["params"]["whitelist"]=[{"name": "ueth"},{"name": "ubtc"},{"name": "uusdc"},{"name": "uusdt"},{"name": "uosmo"},{"name": "uatom"},{"name": "usnp"}]' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["distribution"]["params"]["community_tax"]="0.000000000000000000"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.consensus_params["block"]["max_gas"]="35000000"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.consensus_params["block"]["min_txs_in_block"]="2"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.consensus_params["block"]["max_gas_wanted"]="50000000"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["staking"]["params"]["max_voting_power_ratio"]="1.000000000000000000"' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq '.app_state["bank"]["denom_metadata"]=[{"denom_units":[{"denom":"usnp","exponent":0,"aliases":["USNP"]}],"base":"usnp","display":"usnp","name":"USNP","symbol":"USNP"}]' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
...
cat ~/.snp/config/genesis.json | jq --arg start_date "$START_DATE" --arg end_date "$END_DATE_3DAYS" '.app_state["mint"]["params"]["token_release_schedule"]=[{"start_date": $start_date, "end_date": $end_date, "token_release_amount": "999999999999"}]' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json
cat ~/.snp/config/genesis.json | jq --arg start_date "$END_DATE_3DAYS" --arg end_date "$END_DATE_5DAYS" '.app_state["mint"]["params"]["token_release_schedule"] += [{"start_date": $start_date, "end_date": $end_date, "token_release_amount": "999999999999"}]' > ~/.snp/config/tmp_genesis.json && mv ~/.snp/config/tmp_genesis.json ~/.snp/config/genesis.json

if [ ! -z "$2" ]; then
  APP_TOML_PATH="$2"
else
  APP_TOML_PATH="$HOME/.snp/config/app.toml"
fi
...
if [ ! -z "$1" ]; then
  ...
  CONFIG_PATH="$HOME/.snp/config/config.toml"
fi

if [ ! -z "$2" ]; then
  APP_PATH="$2"
else
  APP_PATH="$HOME/.snp/config/app.toml"
fi
...
GORACE="log_path=/tmp/race/seid_race" ~/go/bin/seid start --trace --chain-id $chainid
