...
func (server msgServer) RegisterPointer(goCtx context.Context, msg *types.MsgRegisterPointer) (*types.MsgRegisterPointerResponse, error) {
	...
	// Retrieve any existing pointer based on the pointer type.
	switch msg.PointerType {
	case types.PointerType_ERC20:
		// For ERC20, the provided address is a hex-encoded Ethereum address.
	...
	case types.PointerType_ERC721:
		// For ERC721, the provided address is a hex-encoded Ethereum address.
	...
	case types.PointerType_NATIVE:
		// Not supported yet.
		panic("NATIVE pointers are not yet supported")
	...
	case types.PointerType_CW1155:
		// Not supported yet.
		panic("CW1155 pointers are not yet supported")
	default:
		panic(fmt.Sprintf("unknown pointer type: %d", msg.PointerType))
	}
	...
	// Build the instantiation payload. Use keys matching the pointer type.
	...
	// If a pointer already exists, migrate it; otherwise instantiate it.
	if exists {
	...
	// Set the pointer mapping and emit the corresponding event.
	switch msg.PointerType {
	case types.PointerType_ERC20:
		...
		ctx.EventManager().EmitEvent(sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeKeyPointerType, "erc20"),
	...
	case types.PointerType_ERC721:
		...
		ctx.EventManager().EmitEvent(sdk.NewEvent(
			types.EventTypePointerRegistered,
			sdk.NewAttribute(types.AttributeKeyPointerType, "erc721"),
			sdk.NewAttribute(types.AttributeKeyPointerAddress, pointerAddr.String()),
			sdk.NewAttribute(types.AttributeKeyPointee, msg.ErcAddress),
	...
	case types.PointerType_ERC1155:
		...
		ctx.EventManager().EmitEvent(sdk.NewEvent(
			types.EventTypePointerRegistered,
			sdk.NewAttribute(types.AttributeKeyPointerType, "erc1155"),
			sdk.NewAttribute(types.AttributeKeyPointerAddress, pointerAddr.String()),
			sdk.NewAttribute(types.AttributeKeyPointee, msg.ErcAddress),
